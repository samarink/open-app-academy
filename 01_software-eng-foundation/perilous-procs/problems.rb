...
def first_index(arr, &prc)
  arr.each do |el|
    return arr.index(el) if prc.call(el)
  end
...
def proc_suffix(sentence, hash)
  ...
  words.each do |word|
    ...
    hash.each do |proc, suffix|
      new_word += suffix if proc.call(word)
    end
...
def procipher(sentence, hash)
  ...
  words.each do |word|
    ...
    hash.each do |filter, modifier|
      if filter.call(word)
        new_word = modifier.call(word)
      end
    end
